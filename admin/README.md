...
| CQRS/package normalization | `api`, `facade`, `application` 기준으로 admin lane이 정리되었고, context별 하위 패키지 분리는 아직 남아 있다. | `com.beat.admin.<context>` 아래 api/facade/application/service/dto 기준으로 더 세분화한다. | #382 |
...
| Async/scheduler handoff | `admin`은 scheduler owner도 no-op schedule booking close job bridge owner도 아니다. | async/coroutine 도입 범위가 결정될 때까지 admin lane의 runtime boundary를 넓히지 않는다. | #383 |

## Current ownership notes

### In `admin`

- admin-facing API controller / facade / application service / DTO
...
- `adapter`, `controller`, `port` 패키지명은 제거되었고, 남은 정리는 context별 세부 패키지 분리다.
...
- 관리자 response 성공 문구가 필요해질 때는 api/response boundary가 소유한다. `SuccessCode`를 domain에 새로 추가하지 않는다.
...
- API Controller는 admin-facing HTTP entrypoint이며 `Facade`만 호출한다.
...
- `application/port/in` 패키지는 도입하지 않는다. Facade가 controller-facing 안정 경계이고, application service가 유스케이스 실행 경계다.
...
- API Controller와 Facade에는 raw Domain model을 절대 올리지 않는다.
...
```text
단일 유스케이스:
API Controller -> Facade -> QueryService -> ResponseDTO

복합 관리자 scenario:
API Controller -> Facade -> QueryService A -> QueryResult A
...
1. `com.beat.admin.<context>` 내부 하위 계층(`api`, `facade`, `application`, `dto`) 정리를 문맥별로 계속 진행
