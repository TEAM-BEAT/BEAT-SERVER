...
@ContextConfiguration(
    classes = [AdminPromotionController::class, AdminPromotionFacade::class, AdminPromotionWebTestConfiguration::class],
)
    ...
    init {
        ...
        test("관리자 Promotion 조회와 upload route가 Application query 결과를 기존 JSON으로 매핑한다") {
            ...
            mockMvc.perform(
                ...
                .andExpect(jsonPath("$.data.carouselPresignedUploads['carousel.png'].imageKey")
                    .value("dev/carousel/carousel.png"))
            mockMvc.perform(get("/api/admin/banner/presigned-url").queryParam("bannerImage", "banner.png"))
                .andExpect(status().isOk)
