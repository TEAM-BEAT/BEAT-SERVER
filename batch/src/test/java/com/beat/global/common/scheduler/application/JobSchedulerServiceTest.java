...
class JobSchedulerServiceTest {
	...
	void registerOrRefreshSchedulesTaskWhenRuntimeOwnsScheduler() {
		...
		com.beat.domain.performance.domain.Performance performance = mock(com.beat.domain.performance.domain.Performance.class);
		ScheduledFuture<Object> scheduledFuture = mock(ScheduledFuture.class);

		when(schedule.getId()).thenReturn(1L);
		when(lockedSchedule.getId()).thenReturn(1L);
