...
class AdminExceptionHttpContractSpec : FunSpec() {
    ...
    private val mockMvc = standaloneSetup(TestController()).setControllerAdvice(handler).build()
        ...
        test("application 에러는 매핑된 상태 코드와 선언된 메시지를 유지한다") {
            val response =
                handler.handleApplicationException(
                    AdminApplicationException(
                        TestApplicationErrorCode(AdminApplicationErrorType.NOT_FOUND)
                    ),
                    MockHttpServletRequest(),
                )
    ...
    private class TestController {
        @PostMapping("/test") fun accept(@RequestBody request: TestRequest) = Unit
    }

    private data class TestRequest(val value: String)

    private data class TestApplicationErrorCode(override val type: AdminApplicationErrorType) :
