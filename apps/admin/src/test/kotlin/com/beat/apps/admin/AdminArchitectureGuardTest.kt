...
/**
 * Admin composition-root contracts. Module-level isolation is enforced by the Gradle graph and
 * Kotlin `internal`; these rules cover composition-root discipline only.
 */
class AdminArchitectureGuardTest :
    FunSpec({
                ...
                object :
                    ...
                    override fun test(input: JavaClass): Boolean {
                        ...
                        return !input.packageName.contains(".config") &&
                            input.simpleName != "AdminApplication"
                    }
                }
            noClasses()
                ...
                .because(
                    "Infrastructure wiring types may only be consumed by admin bootstrap configuration"
                )
                .check(productionClasses)
        }
        ...
        test("admin application 에러 코드는 전체에서 고유하다") {
            val codes =
                PromotionApplicationErrorCode.entries.map { it.code } +
