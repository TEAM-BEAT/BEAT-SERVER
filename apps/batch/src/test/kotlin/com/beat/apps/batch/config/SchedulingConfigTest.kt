 ...
 * 2. 애플리케이션 컨텍스트에 등록된 모든 `@Scheduled` 메서드가 `scheduler`를 명시하는지 — 하드코딩된 메서드 목록이 아니라 컨텍스트 전체를 스캔해
 *    검증하므로, 새 스케줄 작업이 추가되면서 `scheduler` 속성을 빠뜨려도 이 테스트가 잡아낸다. Boot 자동생성 `taskScheduler`용
 *    [org.springframework.boot.task.ThreadPoolTaskSchedulerCustomizer]를 더 이상 두지 않으므로, `scheduler`를
 *    빠뜨린 메서드는 에러 핸들러가 적용되지 않는 이름 없는 스케줄러로 조용히 fallback되어 예외가 관측되지 않을 수 있다.
 */
...
class SchedulingConfigTest : FunSpec() {
    ...
    @Autowired private lateinit var maintenanceTaskScheduler: ThreadPoolTaskScheduler

    @Autowired private lateinit var scheduledTaskErrorHandler: ScheduledTaskErrorHandler
    ...
    init {
        ...
        test("maintenanceTaskScheduler는_지정한_설정으로_구성된다") {
            ...
            ReflectionTestUtils.getField(maintenanceTaskScheduler, "errorHandler") shouldBe
