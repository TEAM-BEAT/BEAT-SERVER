...
class SystemMaintenanceIntegrationSpec : FunSpec() {
    @Autowired private lateinit var ticketCleanupService: TicketCleanupService

    @Autowired private lateinit var promotionMaintenanceService: PromotionMaintenanceService
    ...
    init {
        ...
        test("1년보다 오래된 취소 Booking만 실제 MySQL에서 삭제한다") {
            val oldCancelled =
                bookingRepository.save(
                    booking(BookingStatus.BOOKING_CANCELLED, "2025-08-22T08:59:59")
                )
            val boundaryCancelled =
                bookingRepository.save(
                    booking(BookingStatus.BOOKING_CANCELLED, "2025-08-23T09:00:00")
                )
            val oldConfirmed =
                bookingRepository.save(
                    booking(BookingStatus.BOOKING_CONFIRMED, "2025-01-01T00:00:00")
                )
        ...
        test("만료 Promotion을 삭제하고 남은 carousel 번호를 실제 MySQL에서 연속 재배치한다") {
            val activePerformanceId =
                checkNotNull(performanceRepository.save(performance("active")).id)
            val expiredPerformanceId =
                checkNotNull(performanceRepository.save(performance("expired")).id)
            scheduleRepository.save(
                schedule(activePerformanceId, LocalDate.of(2026, 8, 24), ScheduleNumber.FIRST)
            )
            scheduleRepository.save(
                schedule(expiredPerformanceId, LocalDate.of(2026, 8, 22), ScheduleNumber.FIRST)
            )
            val expired =
                promotionRepository.save(
                    promotion(expiredPerformanceId, CarouselNumber.FIVE, "expired")
                )
            val active =
                promotionRepository.save(
                    promotion(activePerformanceId, CarouselNumber.FOUR, "active")
                )
            ...
            remaining.map { it.carouselNumber } shouldContainExactly
                listOf(CarouselNumber.ONE, CarouselNumber.TWO)
        }
    }
}

private fun booking(status: BookingStatus, cancellationDate: String): Booking =
    Booking.rehydrate(
        id = null,
...
private fun performance(title: String): Performance =
