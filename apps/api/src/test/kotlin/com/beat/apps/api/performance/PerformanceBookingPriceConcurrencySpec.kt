...
private val NOW: LocalDateTime = LocalDateTime.now()
    ...
    @Autowired
    private lateinit var performanceModifyCommandService: PerformanceModifyCommandService
        ...
        context("예매 transaction이 Performance lock을 먼저 획득하면").config(tags = setOf(CORRECTNESS)) {
            test("기존 가격을 snapshot 하고 뒤이은 가격 변경은 거부한다") {
                ...
                try {
                    ...
                    val modifyFuture = executor.submit<Throwable?> {
    ...
    private fun createFixture(): Fixture {
        ...
        val member = memberRepository.save(
            Member.create(
                ...
                userId = makerUserId,
                socialIdentity = SocialIdentity.of(SocialType.KAKAO, 7_000_000_000L + makerUserId),
            ),
        )
    ...
    private fun modifyCommand(fixture: Fixture, ticketPrice: Int): PerformanceModifyCommand =
        PerformanceModifyCommand.of(
            ...
            schedules = listOf(ScheduleModifyCommand.of(fixture.scheduleId, fixture.performanceDate, 10)),
            casts = emptyList(),
            staffs = emptyList(),
            images = emptyList(),
        )

    private fun guestBookingCommand(fixture: Fixture): GuestBookingCommand = GuestBookingCommand.of(
        fixture.scheduleId,
        1,
        "가격검증예매자",
    ...
    private fun assertPersistedSnapshot(fixture: Fixture, expectedPrice: Int) {
        checkNotNull(performanceRepository.findById(fixture.performanceId)).ticketPrice shouldBe expectedPrice
        checkNotNull(scheduleRepository.findById(fixture.scheduleId)).allocatedTicketCount shouldBe 1
    ...
    class TestConfig {
        @Bean
        @Primary
        fun performanceImageStorage(): PerformanceImageStorage = object : PerformanceImageStorage {
