...
@ContextConfiguration(
    classes =
        [
            ...
            TicketFacade::class,
            TicketWebTestConfiguration::class,
            com.beat.apps.api.exception.ApiGlobalExceptionHandler::class,
        ]
)
class TicketControllerWebSpec : FunSpec() {
    ...
    @TestBean private lateinit var ticketQueryService: TicketQueryService

    @TestBean private lateinit var ticketCommandService: TicketCommandService
    ...
    init {
        ...
        test("실제 facade로 매핑된 API 필터로 티켓 목록을 조회한다") {
            ...
            mockMvc
                ...
                .andExpect(
                    jsonPath("$.status").value(TicketSuccessCode.TICKET_RETRIEVE_SUCCESS.status)
                )
                .andExpect(
                    jsonPath("$.message").value(TicketSuccessCode.TICKET_RETRIEVE_SUCCESS.message)
                )
        ...
        test("실제 facade로 매핑된 API 필터로 티켓을 검색한다") {
            ...
            mockMvc
                ...
                .andExpect(
                    jsonPath("$.status").value(TicketSuccessCode.TICKET_SEARCH_SUCCESS.status)
                )
                .andExpect(
                    jsonPath("$.message").value(TicketSuccessCode.TICKET_SEARCH_SUCCESS.message)
                )
                .andExpect(jsonPath("$.data.performanceTitle").value("title"))
                .andExpect(jsonPath("$.data.performanceTeamName").value("team"))
                .andExpect(jsonPath("$.data.totalScheduleCount").value(1))
    ...
    init {
        test("본문 바인딩 실패(누락·null·깨짐)는 400과 표준 메시지로 정규화된다") {
            listOf(
                    ...
                    "명시적 null" to
                        """{"performanceId":1,"bookingList":[{"bookingId":1,"bookingStatus":null}]}""",
                    "깨진 JSON" to """{"broken""",
                )
                .forEach { (_, body) ->
                    mockMvc
    ...
    private companion object {
        ...
        @JvmStatic fun ticketQueryService(): TicketQueryService = mockk()

        @JvmStatic fun ticketCommandService(): TicketCommandService = mockk(relaxed = true)
    }
}
