...
class MemberFacade(
    ...
    fun handleSocialLogin(
        ...
        socialLoginCommandService
            .handleSocialLogin(
                authorizationCode,
                SocialLoginCommand(socialType = request.socialType.toApplicationType()),
            )
            .let { result ->
                MemberLoginSession(
    ...
    fun generateAccessTokenFromRefreshToken(refreshToken: String): AccessTokenGenerateResponse =
        AccessTokenGenerateResponse.from(
            authenticationCommandService.generateAccessTokenFromRefreshToken(refreshToken)
        )

    fun signOut(memberId: Long) = authenticationCommandService.signOut(memberId)

    private fun SocialTypeRequest.toApplicationType(): SocialLoginType =
        when (this) {
