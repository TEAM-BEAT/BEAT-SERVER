...
class BookingController(private val bookingFacade: BookingFacade) : BookingApi {
    ...
    override fun getGuestBookings(
        ...
        val result =
            bookingFacade.findGuestBookings(
        ...
        return ResponseEntity.status(HttpStatus.OK)
            .body(
                SuccessResponse.of(
                    BookingSuccessCode.GUEST_BOOKING_RETRIEVE_SUCCESS,
                    result.response,
                )
            )
    }
    ...
    override fun refundBookings(
        ...
        val response =
            bookingFacade.refundBooking(memberId, guestSessionToken, bookingRefundRequest)
        return ResponseEntity.status(HttpStatus.OK)
    ...
    override fun cancelBookings(
        ...
        val response =
            bookingFacade.cancelBooking(memberId, guestSessionToken, bookingCancelRequest)
        return ResponseEntity.status(HttpStatus.OK)
            .body(SuccessResponse.of(BookingSuccessCode.BOOKING_CANCEL_SUCCESS, response))
    }

    private fun setGuestSessionCookieIfPresent(
        ...
        val cookie =
            ResponseCookie.from(GUEST_SESSION_COOKIE_NAME, sessionToken)
