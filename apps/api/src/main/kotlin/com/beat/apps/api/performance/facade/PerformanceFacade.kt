package com.beat.apps.api.performance.facade
...
class PerformanceFacade(
    ...
    fun createPerformance(memberId: Long, request: PerformanceRequest): PerformanceResponse =
        PerformanceResponse.from(
            createCommandService.createPerformance(memberId, request.toCommand())
        )

    fun modifyPerformance(
        memberId: Long,
        request: PerformanceModifyRequest,
    ): PerformanceModifyResponse =
        PerformanceModifyResponse.from(
            modifyCommandService.modifyPerformance(memberId, request.toCommand())
        )
    ...
    fun getPerformanceEdit(memberId: Long, performanceId: Long): PerformanceModifyDetailResponse =
        PerformanceModifyDetailResponse.from(
            editFormQueryService.getPerformanceEdit(memberId, performanceId)
        )
    ...
    fun getBookingPerformanceDetail(performanceId: Long): BookingPerformanceDetailResponse =
        BookingPerformanceDetailResponse.from(
            detailQueryService.getBookingPerformanceDetail(performanceId)
        )

    fun getMemberPerformances(memberId: Long): MakerPerformanceResponse =
        MakerPerformanceResponse.from(
            makerPerformanceListQueryService.getMemberPerformances(memberId)
        )
...
private fun PerformanceRequest.toCommand(): PerformanceCreateCommand =
    PerformanceCreateCommand.of(
        ...
        images =
            performanceImageList.map {
                PerformanceImageCreateCommand.from(it.performanceImage)
            },
    )

private fun PerformanceModifyRequest.toCommand(): PerformanceModifyCommand =
