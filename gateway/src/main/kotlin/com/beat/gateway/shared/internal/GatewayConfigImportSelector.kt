...
class GatewayConfigImportSelector : DeferredImportSelector {

    override fun selectImports(importingClassMetadata: AnnotationMetadata): Array<String> =
        getValues(importingClassMetadata)
            .map { group -> group.configClass.name }
            .toTypedArray()

    private fun getValues(metadata: AnnotationMetadata): List<GatewayConfigGroup> {
        val attributes = metadata.getAnnotationAttributes(EnableGatewayConfig::class.java.name)
            ?: return emptyList()
