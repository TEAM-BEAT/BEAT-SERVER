...
| `BookingErrorCode` | `NO_TICKETS_FOUND`, `NO_PERFORMANCE_FOUND` | no main-code throw site found in this worktree | unused constants can hide stale API messages | `unused/defer`; do not move without consumer check |
| `TicketErrorCode` | all constants | controller/service validation and ticket update flow | currently application/API policy, not a pure ticket domain model throw | `application-owned` unless follow-up introduces domain ticket model invariant use |
| `MemberErrorCode` | `SOCIAL_TYPE_BAD_REQUEST` | `infra` Kakao adapter | external-provider/auth adapter failure does not belong to domain | `application/infra-auth-owned` follow-up needed |
| `MemberErrorCode`, `UserErrorCode` | `*_NOT_FOUND` | executable services/facades | lookup failures are use-case/persistence orchestration, not domain invariants | `application-owned` |
...
| `PerformanceImageErrorCode`, `CastErrorCode`, `StaffErrorCode` | belong-to-performance / not-found constants | executable performance modification service | relationship checks currently happen in application orchestration; not-found is lookup flow | `application-owned` unless future pure domain relationship model throws them |
