...
class BookingLifecycleInvariantSpec :
    FunSpec({
        ...
        context("메이커의 예매 삭제") {
            ...
            test("유료 확정과 환불 요청 예매는 삭제할 수 없다") {
                ...
                shouldFailWith(BookingErrorCode.DELETION_NOT_ALLOWED) {
        ...
        test("입금 확인 중 예매를 확정하고 동일 상태 전환은 같은 인스턴스를 반환한다") {
            ...
            confirmed.bookingStatus shouldBe BookingStatus.BOOKING_CONFIRMED
            confirmed.transitionTo(BookingStatus.BOOKING_CONFIRMED) should
                beTheSameInstanceAs(confirmed)
        }
        ...
        test("결제 확정은 확정 상태에서 멱등이고 종료 상태에서는 거부한다") {
            ...
            shouldFailWith(BookingErrorCode.PAYMENT_CONFIRMATION_NOT_ALLOWED) {
        ...
        test("환불 완료는 환불 요청 상태만 취소로 전환하고 취소 상태에서는 멱등이다") {
            val requested =
                booking(totalPaymentAmount = 10_000).confirmPayment().requestRefund(refundAccount())
...
private fun booking(totalPaymentAmount: Int = 10_000): Booking =
    bookingFixture(totalPaymentAmount = totalPaymentAmount)

private fun refundAccount(): RefundAccount =
    RefundAccount.of(
