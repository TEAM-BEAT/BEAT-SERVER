...
class PromotionDomainServiceSpec :
    FunSpec({
        ...
        context("캐러셀 배치") {
            test("캐러셀 번호는 중복이나 null 없이 유효해야 한다") {
                carouselService.hasValidCarouselAssignments(
                    listOf(CarouselNumber.ONE, CarouselNumber.THREE)
                ) shouldBe true
                carouselService.hasValidCarouselAssignments(
                    listOf(CarouselNumber.ONE, CarouselNumber.ONE)
                ) shouldBe false
                carouselService.hasValidCarouselAssignments(
                    listOf(CarouselNumber.ONE, null)
                ) shouldBe false
            }
            ...
            test("기존 캐러셀 번호 순서대로 정렬하고 번호를 연속적으로 재배정한다") {
                ...
                arranged.map { it.carouselNumber } shouldBe
                    listOf(CarouselNumber.ONE, CarouselNumber.TWO)
            }

            test("사용 가능한 캐러셀 슬롯보다 많은 프로모션은 배치할 수 없다") {
                val promotions =
                    CarouselNumber.entries
                        .mapIndexed { index, number ->
                            promotion(id = index.toLong(), carouselNumber = number)
                        }
                        .toMutableList()
