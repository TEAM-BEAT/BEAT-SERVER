...
class ScheduleSequenceDomainServiceSpec :
    FunSpec({
        ...
        test("시간순으로 회차 번호를 부여하고 입력 목록은 변경하지 않는다") {
            ...
            assigned.map { it.scheduleNumber } shouldBe
                listOf(ScheduleNumber.FIRST, ScheduleNumber.SECOND)
            input shouldBe listOf(later, earlier)
        }

        test("지원하는 회차 번호보다 많은 회차는 허용하지 않는다") {
            val schedules =
                (0..ScheduleNumber.entries.size).map { index ->
                    sequenceSchedule(index.toLong(), TODAY.plusDays(index.toLong()))
                }

            shouldThrow<DomainException> { service.assignScheduleNumbers(schedules) }
                .errorCode shouldBe ScheduleErrorCode.TOO_MANY_SCHEDULES
        ...
        test("서로 다른 Performance의 회차를 함께 정렬할 수 없다") {
            ...
            shouldThrow<DomainException> { service.assignScheduleNumbers(schedules) }
                .errorCode shouldBe ScheduleErrorCode.MIXED_PERFORMANCE_SCHEDULES
        }
    })
