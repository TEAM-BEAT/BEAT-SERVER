    ...
    FunSpec({
        ...
        context("티켓 재고") {
            ...
            test("해제 수량은 양수이며 할당 수량을 초과할 수 없다") {
                ...
                shouldFailWith(ScheduleErrorCode.NON_POSITIVE_TICKET_COUNT) {
                    schedule.releaseTickets(0)
                }
                shouldFailWith(ScheduleErrorCode.EXCESS_TICKET_DELETE) {
                    schedule.releaseTickets(4)
                }
            }

            test("잔여 수량은 전체 수량에서 할당 수량을 뺀 값이다") {
                schedule(totalTicketCount = 10, allocatedTicketCount = 3)
                    .availableTicketCount shouldBe 7
            }
        ...
        context("예매 종료 시각") {
            test("공연 연장에 맞춰 뒤로 변경할 수 있다") {
                ...
                schedule.updateBookingCloseAt(extendedCloseAt).bookingCloseAt shouldBe
                    extendedCloseAt
            }
