...
class DomainEntityEqualityBehaviorTest : FunSpec() {
        ...
        test("영속 Entity는 식별자로, transient Entity는 같은 instance로만 동등하다") {
            entityCases().forEach { case ->
                ...
                withClue("${case.name} hash contract") {
                    case.persisted.hashCode() shouldBe
    ...
    private fun entityCases(): List<EntityCase> =
        listOf(
            EntityCase(
                "Booking",
                booking(1, "first"),
    ...
    private fun booking(id: Long, name: String): Booking =
        Booking.rehydrate(
            id,
            1,
            name,
    ...
    private fun newBooking(): Booking =
        bookingFixture(
            bookerName = "name",
    ...
    private fun performance(id: Long, title: String): Performance =
    ...
    private fun newPerformance(): Performance =
        Performance.create(
            performanceTitle = "title",
            genre = Genre.PLAY,
            runningTime = RunningTime.of(90),
    ...
    private fun newPromotion(): Promotion =
        Promotion.create("photo", null, "url", true, CarouselNumber.ONE)

    private fun schedule(id: Long, ticketCount: Int): Schedule =
        Schedule.rehydrate(
            id,
