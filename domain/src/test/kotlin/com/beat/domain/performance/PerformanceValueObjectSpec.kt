...
class PerformanceValueObjectSpec :
    FunSpec({
        ...
        context("결제 계좌") {
            ...
            test("은행, 계좌번호, 예금주가 모두 있으면 생성할 수 있다") {
                PaymentAccount.fromNullable(BankName.KAKAOBANK, "123", "holder")!!.bankName shouldBe
                    BankName.KAKAOBANK
            }
            ...
            test("일부 값만 있거나 유효하지 않은 값이면 생성할 수 없다") {
                ...
                shouldThrow<DomainException> {
                    PaymentAccount.of(BankName.KAKAOBANK, " ", "holder")
                }
        ...
        context("공연 기간") {
            test("회차들의 가장 이른 날짜와 가장 늦은 날짜를 사용한다") {
                val period =
                    PerformancePeriod.fromPerformanceDateTimes(
                        listOf(
