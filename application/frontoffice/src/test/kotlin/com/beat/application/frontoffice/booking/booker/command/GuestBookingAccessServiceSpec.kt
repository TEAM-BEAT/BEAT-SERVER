...
class GuestBookingAccessServiceSpec :
    FunSpec({
        test("게스트 인증과 예매 조회를 마치면 throttle을 초기화하고 session을 발급한다") {
            ...
            every { dependencies.historyPort.findByUserId(USER_ID) } returns
                listOf(bookingReadModel())
        ...
        test("게스트 예매 조회 후 session 발급이 실패해도 조회 결과를 반환한다") {
            ...
            every { dependencies.historyPort.findByUserId(USER_ID) } returns
                listOf(bookingReadModel())
            every { dependencies.sessionStore.issue(USER_ID) } throws
...
private fun bookingReadModel(): BookingHistorySnapshot =
