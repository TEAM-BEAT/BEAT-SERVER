...
class TicketCommandApplicationSpec : FunSpec() {
    ...
    init {
            ...
            shouldThrow<FrontofficeApplicationException> {
                service()
                    .updateTickets(
                        ...
                        TicketUpdateCommand(
                            100L,
                            listOf(statusUpdate(300L, TicketBookingStatus.BOOKING_CONFIRMED)),
                        ),
                    )
            }
        ...
        test("저장된 booking payload로 결제 확인 event를 발행한다") {
            val booking =
                booking(id = 300L, scheduleId = 200L, status = BookingStatus.CHECKING_PAYMENT)
            stubOwner(booking)
                ...
                shouldThrow<FrontofficeApplicationException> {
                    service()
                        .updateTickets(
                            ...
                            TicketUpdateCommand(
                                100L,
                                listOf(statusUpdate(300L, TicketBookingStatus.BOOKING_CANCELLED)),
                            ),
                        )
                }
        ...
        test("환불 완료 시 할당된 티켓을 반납한다") {
            val booking =
                booking(status = BookingStatus.REFUND_REQUESTED, totalPaymentAmount = 10_000)
            stubOwner(booking)
        ...
        test("환불 완료는 v1 message와 타입을 유지하며 유효하지 않은 상태를 변환한다") {
            val booking =
                booking(status = BookingStatus.BOOKING_CONFIRMED, totalPaymentAmount = 10_000)
            stubOwner(booking)

            val exception =
                shouldThrow<FrontofficeApplicationException> {
                    service()
                        .refundTicketsByBookingIds(1L, TicketBookingIdsCommand(100L, listOf(300L)))
                }
        ...
        test("삭제는 미입금 예매의 티켓 할당을 반납한다") {
            val booking =
                booking(status = BookingStatus.CHECKING_PAYMENT, totalPaymentAmount = 10_000)
            stubOwner(booking)
        ...
        test("삭제는 v1 message와 타입을 유지하며 유효하지 않은 상태를 변환한다") {
            ...
            val exception =
                shouldThrow<FrontofficeApplicationException> {
                    service()
                        .deleteTicketsByBookingIds(1L, TicketBookingIdsCommand(100L, listOf(300L)))
                }
    ...
    private fun savedBooking(): Booking = savedBookings.last()

    private fun savedSchedule(): Schedule = savedSchedules.last()

    private fun hasInvocation(repository: ScheduleRepository): Boolean = savedSchedules.isNotEmpty()
    ...
    private fun member(userId: Long = 10L) =
    ...
    private fun performance(userId: Long = 10L) =
    ...
    private fun schedule(id: Long, performanceId: Long = 100L) =
        frontofficeScheduleFixture(
            id = id,
