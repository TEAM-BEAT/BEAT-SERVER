...
class HomeQueryApplicationSpec :
    FunSpec({
        ...
        test("공연 기간 형식과 dueDate 정책에 따라 비만료 공연, 미지정 회차, 만료 공연 순으로 정렬한다") {
            ...
            result.performanceList.map { it.performancePeriod } shouldBe
                listOf(
...
private class RecordingHomeProjectionReader(private val projection: HomeProjection) :
