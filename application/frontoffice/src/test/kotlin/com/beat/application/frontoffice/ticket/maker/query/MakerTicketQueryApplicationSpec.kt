...
class MakerTicketQueryApplicationSpec : FunSpec() {
    init {
        test("명시적인 회차와 예매 상태 필터를 매핑하고 null이 아닌 결과를 반환한다") {
            val (makerTicketReader, performanceRepository, memberRepository) =
                makerTicketDependencies()
                ...
                shouldThrow<FrontofficeApplicationException> {
                    service(makerTicketReader, performanceRepository, memberRepository)
                        .searchAllTicketsByConditions(
                            1L,
                            100L,
                            TicketListQuery(searchWord = searchWord),
                        )
                }
            }
                ...
                shouldThrow<FrontofficeApplicationException> {
                    service(makerTicketReader, performanceRepository, memberRepository)
                        .searchAllTicketsByConditions(
                            ...
                            TicketListQuery(
                                searchWord = "ab",
                                bookingStatuses = listOf("BOOKING_DELETED"),
                            ),
                        )
                }
