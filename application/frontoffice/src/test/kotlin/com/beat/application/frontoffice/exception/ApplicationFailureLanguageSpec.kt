...
class ApplicationFailureLanguageSpec :
    FunSpec({
        ...
        context("회원과 토큰 Application failure language에서") {
            test("error code는 명시적인 형식으로 중복 없이 유지된다") {
                val codes =
                    MemberApplicationErrorCode.entries.map { it.code } +
        ...
        test("domain failure는 전체 special mapping과 generic mapping을 보존한다") {
            val dataFormatMessageCodes =
                setOf<DomainErrorCode>(
            ...
            allDomainCodes.forEach { domainCode ->
                ...
                val expectedType =
                    when {
                        domainCode in invalidInputTypeCodes ->
                            FrontofficeApplicationErrorType.INVALID_INPUT
                        domainCode == PerformanceErrorCode.DELETE_NOT_ALLOWED ->
                            FrontofficeApplicationErrorType.FORBIDDEN
                        domainCode.type == DomainErrorType.INVALID_INPUT ->
                            FrontofficeApplicationErrorType.INVALID_INPUT
