...
/**
 * Capability → Actor → CQRS semantics inside the frontoffice lane. These rules encode design intent
 * that neither the Gradle graph (single module) nor Kotlin visibility can express: command/query
 * separation, actor lanes, cross-capability isolation, auth collaboration boundaries, and failure
 * translation.
 */
...
class FrontofficeApplicationArchitectureTest :
    FunSpec({
        ...
        fun hasPackage(pattern: PackagePattern): Boolean = importedClasses.any { javaClass ->
            javaClass.packageName.split('.').windowed(pattern.segments.size).any {
                it == pattern.segments
            }
        }
        ...
        test("frontoffice Application은 기술 구현에 의존하지 않는다") {
            noClasses()
                ...
                .because(
                    "Frontoffice Application technology boundary — jOOQ and generated types are infrastructure only (E-08)"
                )
                .check(importedClasses)
        }
        ...
        test("frontoffice Application은 infrastructure jOOQ generated 타입에 의존하지 않는다") {
            noClasses()
                ...
                .because(
                    "jOOQ type leakage = 0 — generated types must not escape infrastructure (I-21)"
                )
                .check(importedClasses)
        }
