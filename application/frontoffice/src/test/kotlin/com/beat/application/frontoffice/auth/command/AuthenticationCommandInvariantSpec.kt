...
class AuthenticationCommandInvariantSpec :
    FunSpec({
        ...
        context("refresh token 거부 매핑") {
            test("모든 보안 실패를 정확한 application 에러로 매핑한다") {
                val expectedCodes =
                    mapOf(
                        TokenAuthenticationFailure.EXPIRED to
                            TokenApplicationErrorCode.REFRESH_TOKEN_EXPIRED_ERROR,
                        TokenAuthenticationFailure.INVALID_TOKEN to
                            TokenApplicationErrorCode.INVALID_REFRESH_TOKEN_ERROR,
                        TokenAuthenticationFailure.INVALID_SIGNATURE to
                            TokenApplicationErrorCode.REFRESH_TOKEN_SIGNATURE_ERROR,
                        TokenAuthenticationFailure.UNSUPPORTED to
                            TokenApplicationErrorCode.UNSUPPORTED_REFRESH_TOKEN_ERROR,
                        TokenAuthenticationFailure.EMPTY to
                            TokenApplicationErrorCode.REFRESH_TOKEN_EMPTY_ERROR,
                    )

                expectedCodes.forEach { (failure, expectedCode) ->
                    val dependencies = AuthenticationDependencies()
                    every {
                        dependencies.refreshTokenAuthenticator.authenticateRefreshToken(
                            REFRESH_TOKEN
                        )
                    } returns TokenAuthenticationResult.Rejected(failure)
                    ...
                    verify {
                        listOf(dependencies.tokenIssuer, dependencies.refreshTokenStore) wasNot
                            Called
                    }
                }
            }
            ...
            test("저장된 소유자 확인이나 access token 발급 전에 유효하지 않은 token을 거부한다") {
                val dependencies = AuthenticationDependencies()
                every {
                    dependencies.refreshTokenAuthenticator.authenticateRefreshToken(REFRESH_TOKEN)
                } returns
        ...
        context("저장된 refresh token 소유자 검증") {
            test("저장된 member가 없으면 refresh token을 거부한다") {
                ...
                every {
                    dependencies.refreshTokenStore.findMemberIdByRefreshToken(REFRESH_TOKEN)
                } returns null
            ...
            test("저장된 소유자가 token subject와 다르면 refresh token을 거부한다") {
                ...
                exception.errorCode shouldBe
                    TokenApplicationErrorCode.REFRESH_TOKEN_MEMBER_ID_MISMATCH_ERROR
        ...
        context("access token 발급") {
            test("인증과 저장된 소유자 확인 후 access token을 발급한다") {
                ...
                every {
                    dependencies.refreshTokenAuthenticator.authenticateRefreshToken(REFRESH_TOKEN)
                } returns TokenAuthenticationResult.Authenticated(subject)
                every {
                    dependencies.refreshTokenStore.findMemberIdByRefreshToken(REFRESH_TOKEN)
                } returns MEMBER_ID
        ...
        context("sign-out 멱등성") {
            ...
            test("반복 삭제 시도에서도 멱등을 유지한다") {
                val dependencies = AuthenticationDependencies()
                every { dependencies.refreshTokenStore.delete(MEMBER_ID) } returnsMany
                    listOf(true, false)
...
private class AuthenticationDependencies {
    ...
    val service =
        AuthenticationCommandService(tokenIssuer, refreshTokenAuthenticator, refreshTokenStore)
}
