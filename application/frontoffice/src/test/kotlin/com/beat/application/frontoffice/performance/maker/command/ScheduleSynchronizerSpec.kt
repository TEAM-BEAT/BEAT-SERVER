...
@Tags("correctness")
class ScheduleSynchronizerSpec :
    FunSpec({
        test("활성 예매 확인 전에 정렬된 순서로 서로 다른 회차를 잠근다") {
            ...
            every { scheduleRepository.lockById(1L) } returns
                frontofficeScheduleFixture(id = 1L, performanceId = 9L)
            every { scheduleRepository.lockById(2L) } returns
                frontofficeScheduleFixture(id = 2L, performanceId = 9L)
            every { scheduleRepository.lockById(3L) } returns
                frontofficeScheduleFixture(id = 3L, performanceId = 9L)
            every {
