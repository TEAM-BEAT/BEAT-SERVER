...
class GuestBookingCredentialLookupSpec :
    FunSpec({
        ...
        test("비밀번호가 일치하는 credential이 없으면 userId를 반환하지 않는다") {
            ...
            val credentialStore = mockk<GuestBookingCredentialStore>(relaxed = true)
            every { credentialStore.findCandidates("booker", "010", "990101") } returns
                listOf(GuestBookingCredential(1L, "encoded"))
            every { passwordHasher.matches("secret", "encoded") } returns false

            val result =
                authenticator(passwordHasher, credentialStore)
                    .findUserId("booker", "010", "990101", "secret")

            result shouldBe null
            verify(exactly = 0) { credentialStore.replaceEncodedPassword(any(), any()) }
        }
        ...
        test("한 user의 여러 credential이 일치하면 upgrade를 한 번만 수행한다") {
            ...
            result shouldBe 7L
            verify { credentialStore.replaceEncodedPassword(7L, "upgraded") }
...
private fun authenticator(
    ...
    credentialStore: GuestBookingCredentialStore,
): GuestBookingCredentialAuthenticator =
    GuestBookingCredentialAuthenticator(
        passwordHasher = passwordHasher,
        guestBookingCredentialStore = credentialStore,
    )
