...
internal constructor(private val scheduleRepository: ScheduleRepository) {
        ...
        return translateDomainFailure {
            if (scheduleId <= 0 || purchaseTicketCount <= 0) {
                throw FrontofficeApplicationException(
                    ScheduleApplicationErrorCode.INVALID_TICKET_AVAILABILITY_REQUEST
                )
            }
            val schedule =
            ...
            if (!schedule.canPurchase(purchaseTicketCount)) {
                throw FrontofficeApplicationException(
                    ScheduleApplicationErrorCode.INSUFFICIENT_TICKETS
                )
            }
            TicketAvailabilityResult(
