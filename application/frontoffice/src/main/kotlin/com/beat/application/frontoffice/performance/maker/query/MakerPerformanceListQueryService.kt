...
internal constructor(
    ...
    fun getMemberPerformances(memberId: Long): MakerPerformanceListResult {
        return translateDomainFailure {
            ...
            val details =
                makerPerformanceListReader.findByUserId(member.userId).map { performance ->
                    val minDueDate =
                        performance.representativePerformanceDate?.let {
                            calculateDueDate(today, it)
                        } ?: Int.MAX_VALUE
            ...
            val sorted =
                details.filter { it.minDueDate >= 0 }.sortedBy { it.minDueDate } +
