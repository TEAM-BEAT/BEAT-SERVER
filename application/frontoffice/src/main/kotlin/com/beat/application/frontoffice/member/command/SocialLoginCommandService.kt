...
@Service
class SocialLoginCommandService
internal constructor(
    ...
    ): LoginSuccessResult = translateDomainFailure {
        val socialType = SocialType.valueOf(command.socialType.name)
        val socialLoginProfile =
            try {
        ...
        val memberAndRole =
            try {
                        ...
                        val user =
                            userRepository.findById(member.userId)
                                ?: throw FrontofficeApplicationException(
                                    MemberApplicationErrorCode.USER_NOT_FOUND
                                )
                ...
                requireNotNull(
                    transactions.execute {
                        val member =
                            socialLoginMemberResolver.findExisting(socialIdentity)
                                ?: throw duplicate
                        val user =
                            userRepository.findById(member.userId)
                                ?: throw FrontofficeApplicationException(
    ...
    private fun SocialType.toLoginType(): SocialLoginType =
        when (this) {
    ...
    private fun SocialLoginFailure.toApplicationException(): FrontofficeApplicationException {
        val errorCode =
            when (reason) {
                ...
                SocialLoginFailure.Reason.AUTHENTICATION_FAILED ->
                    MemberApplicationErrorCode.AUTHENTICATION_CODE_EXPIRED
                SocialLoginFailure.Reason.PROVIDER_FAILURE ->
                    MemberApplicationErrorCode.SOCIAL_LOGIN_PROVIDER_FAILURE
                SocialLoginFailure.Reason.PROVIDER_UNAVAILABLE ->
                    MemberApplicationErrorCode.SOCIAL_LOGIN_PROVIDER_UNAVAILABLE
                SocialLoginFailure.Reason.PROVIDER_TIMEOUT ->
