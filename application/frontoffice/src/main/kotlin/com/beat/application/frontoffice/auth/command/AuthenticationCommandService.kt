...
internal constructor(
    ...
    private fun authenticateRefreshToken(refreshToken: String) =
        when (val result = refreshTokenAuthenticator.authenticateRefreshToken(refreshToken)) {
            is TokenAuthenticationResult.Authenticated -> result.subject
            is TokenAuthenticationResult.Rejected ->
                throw FrontofficeApplicationException(result.failure.toErrorCode())
        }

    private fun verifyStoredTokenOwner(refreshToken: String, memberId: Long) {
        val storedMemberId =
            refreshTokenStore.findMemberIdByRefreshToken(refreshToken)
                ?: throw FrontofficeApplicationException(
                    TokenApplicationErrorCode.REFRESH_TOKEN_NOT_FOUND
                )
    ...
    private fun TokenAuthenticationFailure.toErrorCode(): TokenApplicationErrorCode =
        when (this) {
            TokenAuthenticationFailure.EXPIRED ->
                TokenApplicationErrorCode.REFRESH_TOKEN_EXPIRED_ERROR
            TokenAuthenticationFailure.INVALID_TOKEN ->
                TokenApplicationErrorCode.INVALID_REFRESH_TOKEN_ERROR
            TokenAuthenticationFailure.INVALID_SIGNATURE ->
                TokenApplicationErrorCode.REFRESH_TOKEN_SIGNATURE_ERROR
            TokenAuthenticationFailure.UNSUPPORTED ->
