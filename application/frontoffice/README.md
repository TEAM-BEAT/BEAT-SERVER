...
`application:frontoffice`는 BEAT 프론트오피스(예매자/공연 메이커)의 **Application Service 및 유스케이스 오케스트레이션 레이어**입니다.
CQRS 패턴을 기반으로 예매자(Booker)와 공연 메이커(Maker)의 요구사항을 명확히 분리하여 처리합니다.
...
| `performance.booker.query` | 예매자용 공연 상세 정보 및 예매 가능 회차 조회 |
...
- **액터별 유스케이스 격리**: 예매자(Booker)와 공연 메이커(Maker)의 요구사항을 독립적인 패키지로 격리
...
- **영속성 구현 세부사항**: JPA Entity, Spring Data Repository, jOOQ/JDSL 쿼리 구현은 `:infrastructure`가 소유
