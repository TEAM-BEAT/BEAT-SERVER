...
class PromotionMaintenanceApplicationSpec :
    FunSpec({
...
private fun fixture(
    ...
    val performanceRepository =
        RecordingSystemPerformanceRepository(performanceIds.associateWith(::performance))
...
private class RecordingSystemScheduleRepository(schedules: List<Schedule>) : ScheduleRepository {
    ...
    override fun findAllByPerformanceId(performanceId: Long): List<Schedule> =
        schedules.values.filter { it.performanceId == performanceId }
    ...
    override fun countByPerformanceId(performanceId: Long): Int =
        findAllByPerformanceId(performanceId).size
...
private fun schedule(id: Long, performanceId: Long, date: LocalDate): Schedule =
    Schedule.rehydrate(
        id = id,
...
private val SYSTEM_CLOCK: Clock =
    Clock.fixed(
