...
class AdminPromotionCommandApplicationSpec : FunSpec({
    ...
    context("캐러셀 요청이 유효하지 않으면") {
        test("중복 carousel 번호를 저장소 mutation 전에 거부한다") {
            ...
            val command = CarouselHandleCommand(
                listOf(
                    PromotionGenerateCommand("ONE", "prod/carousel/one", true, "url-1", null),
                    PromotionGenerateCommand("ONE", "prod/carousel/two", true, "url-2", null),
                ),
            )
        ...
        test("중복 수정 id를 저장소 mutation 전에 거부한다") {
            ...
            val command = CarouselHandleCommand(
                listOf(
                    PromotionModifyCommand(1L, "ONE", "prod/carousel/one", true, "url-1", null),
                    PromotionModifyCommand(1L, "TWO", "prod/carousel/two", true, "url-2", null),
                ),
            )
    ...
    context("업로드된 캐러셀 이미지가 존재하지 않으면") {
        test("Performance와 Promotion을 lock하기 전에 실패한다") {
            ...
            val command = CarouselHandleCommand(
                listOf(PromotionGenerateCommand("ONE", "prod/carousel/missing", false, "url", PERFORMANCE_ID)),
            )
    ...
    context("연결할 Performance가 존재하지 않으면") {
        test("authoritative Performance lock에서 실패하고 Promotion을 변경하지 않는다") {
            ...
            val command = CarouselHandleCommand(
                listOf(PromotionGenerateCommand("ONE", "prod/carousel/new", false, "url", PERFORMANCE_ID)),
            )
    ...
    context("유효한 캐러셀 일괄 변경이면") {
        test("Performance를 정렬된 순서로 lock하고 삭제·수정·생성·응답 정렬을 수행한다") {
            val existing = promotion(1L, "old", PERFORMANCE_ID, "old-url", false, CarouselNumber.TWO)
            ...
            val command = CarouselHandleCommand(
                listOf(
                    ...
                    PromotionGenerateCommand("ONE", "prod/carousel/created", false, "created-url", null),
                ),
            )

            val result = fixture.service.processAllPromotionsSortedByCarouselNumber(MEMBER_ID, command)

            fixture.performances.lockedIds shouldContainExactly listOf(PERFORMANCE_ID)
            fixture.promotions.deletedIds shouldContainExactly listOf(2L)
            fixture.cache.warmedKeys shouldContainExactly listOf("prod/carousel/modified", "prod/carousel/created")
            result.promotionResults.map { it.carouselNumber } shouldContainExactly listOf("ONE", "THREE")
        ...
        test("여러 Performance reference를 중복 제거하고 id 오름차순으로 lock한다") {
            val fixture = commandFixture(
                ...
                existingImages = setOf("prod/carousel/one", "prod/carousel/two", "prod/carousel/three"),
            )
            val command = CarouselHandleCommand(
                listOf(
                    PromotionGenerateCommand("ONE", "prod/carousel/one", false, "one", 20L),
                    PromotionGenerateCommand("TWO", "prod/carousel/two", false, "two", 10L),
                    PromotionGenerateCommand("THREE", "prod/carousel/three", false, "three", 20L),
                ),
            )
...
private fun commandFixture(
    ...
    existingImages: Set<String> = setOf("prod/carousel/new", "prod/carousel/one", "prod/carousel/two"),
): CommandFixture {
...
private class StubPromotionMemberRepository(
...
private class RecordingPromotionRepository(
    initialPromotions: List<Promotion>,
) : PromotionRepository {
    ...
    override fun findById(promotionId: Long): Promotion? =
        promotions.firstOrNull { it.id == promotionId }
    override fun save(promotion: Promotion): Promotion {
        val savedPromotion = if (promotion.id == null) {
            promotion(
                id = nextId++,
    ...
    override fun findByCarouselNumber(carouselNumber: CarouselNumber): Promotion? =
        promotions.firstOrNull { it.carouselNumber == carouselNumber }
}
...
private class StubPromotionImageStorage(
    ...
    override fun issueBannerUpload(imageName: String): PromotionImageUpload = PromotionImageUpload("url", imageName)
