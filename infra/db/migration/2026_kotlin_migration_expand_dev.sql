-- =============================================================================
-- BEAT Kotlin 마이그레이션 선행 스키마 EXPAND — beatDev
-- MySQL 8.0 / 멱등(재실행 안전) / DDL·DML·DCL·TCL 만 사용 (클라이언트 메타 명령 없음)
-- =============================================================================
-- 이 파일은 infra/db/migration/2026_kotlin_migration_expand_prod.sql 과 반드시
...
--     (2026_kotlin_migration_contract_dev.sql, 롤백 윈도우 이후 별도 실행)
--
-- 안전장치:
...
--
-- 실행 (이 파일이 연결된 beatDev 커넥션에서 그대로):
--   mysql -h <dev-db-endpoint> -u <user> -p beatDev < infra/db/migration/2026_kotlin_migration_expand_dev.sql
--
-- 배포 순서(반드시 지킬 것):
--   [1] 이 dev EXPAND 실행 → [2] Kotlin 마이그레이션 앱을 dev에 배포·관측
--   → [3] prod EXPAND(2026_kotlin_migration_expand_prod.sql) 실행 → [4] prod 배포
--   → [5] 롤백 윈도우 종료 후 CONTRACT(NOT NULL) 별도 실행
-- =============================================================================
