...
그 아래에서 `@Import`로 전이 로드되는 support config(`TaskExecutorConfig`, `ThreadPoolProperties`, `InfraPersistenceConfig`, `S3InfraConfig`)는
...
- support config(`TaskExecutorConfig`, `ThreadPoolProperties`, `InfraPersistenceConfig`, `S3InfraConfig`)에 `InfraBaseConfig` 구현 금지
