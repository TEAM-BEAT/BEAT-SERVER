...
| Infra JPA entity | `infra/src/main/java/com/beat/infra/persistence/promotion/entity/PromotionJpaEntity.java` | `promotion` table, `performance_id` FK column, column/enum mapping을 소유한다. `Performance` 객체 연관관계는 두지 않는다. |
...
현재 baseline은 순수 domain `Promotion.kt`와 Java `PromotionJpaEntity`를 최종 상태로 유지한다. `PromotionJpaEntity`의 Kotlin 변환은 이번 stabilization에 섞지 않고, 별도 실험 PR에서 아래 기준을 모두 통과할 때만 후보로 본다.

Scope:

- 변환 대상은 `infra/src/main/java/com/beat/infra/persistence/promotion/entity/PromotionJpaEntity.java` 하나로 제한한다.
...
- entity name, table/column mapping, nullable `performance_id`, `CarouselNumber` enum string mapping, protected no-arg constructor semantics, `rehydrate(...)` factory contract를 보존한다.
...
2. Kotlin 변환 후 Java callers가 기존 static factory와 getter contract를 그대로 사용할 수 있어야 한다. 필요하면 `@JvmStatic`, Java-visible accessor naming, constructor visibility를 명시적으로 검증한다.
3. JPA가 reflection으로 entity를 생성할 수 있어야 한다. noarg/all-open 없이 불가능하면 실험 결과는 실패로 기록하고 Java entity를 유지한다.
