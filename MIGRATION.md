...
| Query/read-model contract | `module-contracts/src/main/java/com/beat/contracts/schedule/readmodel/MinPerformanceDate.java`, `ScheduleReadPort.java` | #420 기준 최소 공연 일자 조회 read-model contract이며 domain type을 import하지 않는다 | query/projection 구현은 `infra`, 실행 모듈은 `module-contracts` read port 사용 |
...
| `Schedule` | inventory/seat count, pessimistic lock, read-model contract `MinPerformanceDate`, custom repository implementation, `JpaConfig` scan 변경과 직접 결합된다. | 첫 slice로는 금지했으며, #415에서 pure domain + infra persistence/query adapter seam으로 분리했다. |
...
- 최소 공연 일자 조회 read-model은 `module-contracts/src/main/java/com/beat/contracts/schedule/readmodel/MinPerformanceDate.java`와 `ScheduleReadPort.java`가 소유하며, domain repository/dto 패키지에는 두지 않습니다.
