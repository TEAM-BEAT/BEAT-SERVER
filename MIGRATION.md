...
| JPA entity / relation mapping | `Booking`, `Member`, `Performance`, `PerformanceImage` under `domain/src/main/java/com/beat/domain/*/domain`; `Promotion`, #404 `Cast`/`Staff`, #409 `Users`, and #415 `Schedule` table mapping belong under `infra.persistence.<context>.entity` | DB identity, column/enum mapping, relation/cascade/delete mapping | `infra.persistence.<context>.entity` |
| Spring Data repository adapter | `BookingRepository`, `TicketRepository`, `MemberRepository`, `PerformanceImageRepository`, `PerformanceRepository` under `domain/**/dao`; `ScheduleRepository` is now a technology-neutral domain port under `domain/schedule/repository`, with #415 `ScheduleJpaRepository`/`ScheduleRepositoryImpl` in `infra/persistence/schedule/repository` | `JpaRepository`, JPQL `@Query`, locking/modifying/transaction annotations, derived query methods | `infra.persistence.<context>.repository` |
| Mixed custom/query contracts | `TicketRepositoryCustom`, `MemberRepositoryCustom` | 현재는 domain package에 있지만 query/adapter 성격이 섞여 있음. `ScheduleRepositoryCustom`은 #415에서 제거되고 query adapter가 `infra.persistence.schedule.repository.query`로 이동함 | business contract이면 domain port, query/adapter이면 infra; #381/slice PR에서 판정 |
...
- `Booking`, `Member`, `Performance` 엔티티는 더 이상 `Users` JPA 엔티티를 직접 참조하지 않고 `Long userId` 스칼라 값을 가집니다.
