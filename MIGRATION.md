...
| JPA entity / relation mapping | `Booking`, `Member`, `Performance`, `PerformanceImage`, `Schedule` under `domain/src/main/java/com/beat/domain/*/domain`; `Promotion` mapping moved to `infra.persistence.promotion.entity.PromotionJpaEntity`; `Cast`/`Staff` and `Users` are pure domain models and their table mapping belongs under `infra.persistence.{cast,staff,user}.entity` | DB identity, column/enum mapping, relation/cascade/delete mapping | `infra.persistence.<context>.entity` |
| Spring Data repository adapter | `BookingRepository`, `TicketRepository`, `MemberRepository`, `PerformanceImageRepository`, `PerformanceRepository`, `ScheduleRepository` under `domain/**/dao`; `PromotionJpaRepository`, #404 `CastJpaRepository`/`StaffJpaRepository`, and #409 `UsersJpaRepository` live under `infra/persistence/**/repository` | `JpaRepository`, JPQL `@Query`, locking/modifying/transaction annotations, derived query methods | `infra.persistence.<context>.repository` |
