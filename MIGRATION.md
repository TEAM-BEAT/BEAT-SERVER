...
| JPA entity / relation mapping | `Booking`, `Member`, `Performance`, `PerformanceImage`, `Schedule`, `Users` under `domain/src/main/java/com/beat/domain/*/domain`; `Promotion` mapping moved to `infra.persistence.promotion.entity.PromotionJpaEntity`; `Cast`/`Staff` are #404 pure domain models and their table mapping belongs under `infra.persistence.{cast,staff}.entity` | DB identity, column/enum mapping, relation/cascade/delete mapping | `infra.persistence.<context>.entity` |
| Spring Data repository adapter | `BookingRepository`, `TicketRepository`, `MemberRepository`, `PerformanceImageRepository`, `PerformanceRepository`, `ScheduleRepository`, `UserRepository` under `domain/**/dao`; `PromotionJpaRepository` and #404 `CastJpaRepository`/`StaffJpaRepository` live under `infra/persistence/**/repository` | `JpaRepository`, JPQL `@Query`, locking/modifying/transaction annotations, derived query methods | `infra.persistence.<context>.repository` |
