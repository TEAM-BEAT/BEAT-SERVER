...
apis/
  src/main/kotlin/com/beat/apis/
    ...
    config/
      ...
      InfraConfig.kt                # @EnableInfraBaseConfig(..., AUTH_REDIS)
...
- `GatewayConfig`가 `@EnableGatewayServletSecurity`와 `@EnableGatewayConfig(REFRESH_TOKEN_STORE, GUEST_ACCESS)`를 캡슐화하고, `InfraConfig`가 실제 `AUTH_REDIS` 저장 adapter를 선택한다.
...
| Gateway/Redis boundary | `@EnableGatewayServletSecurity`, `@EnableGatewayConfig(REFRESH_TOKEN_STORE, GUEST_ACCESS)`, `gateway.CurrentMember` 공개 표면을 사용한다. `REFRESH_TOKEN_STORE`는 port 제공을 검증하고 실제 Redis 구현은 `InfraBaseConfigGroup.AUTH_REDIS`로 선택한다. | gateway/infra 내부 패키지 직접 참조 없이 port와 bootstrap group만 사용한다. | architecture guard로 고정 |
