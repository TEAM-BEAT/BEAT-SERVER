...
- `ApisApplication`은 정확히 아래 bootstrap surface만 import한다.
    - `@EnableGatewayConfig(SERVLET_SECURITY, REFRESH_TOKEN_STORE)`
    - `InfraConfig`
...
| Gateway boundary | `@EnableGatewayConfig(SERVLET_SECURITY, REFRESH_TOKEN_STORE)`와 `gateway.security.servlet.CurrentMember` 공개 표면으로 인증/refresh-token 경계를 선택한다. | gateway 내부 패키지 직접 참조 없이 필요한 gateway group만 선택한다. | #437에서 guard 고정 |
...
- `ApisArchitectureGuardTest`
    ...
    - API client boundary(`api`, `facade`, `application/dto`, `request`, `response`, `result`)의 domain enum/value import 금지
...
- query service는 ResponseDTO를 조립하지만 Domain model을 Facade나 다른 ApplicationService로 반환하지 않는다.
