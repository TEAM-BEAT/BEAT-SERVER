...
class ApisJsonCompatibilitySpec : FunSpec({
    ...
    test("api 경계 마이그레이션 전후로 api enum 이름이 호환된다") {
        withClue("API enum names") {
            ...
            enumNames(HomeGenreType.entries.toTypedArray()) shouldBe listOf("BAND", "PLAY", "DANCE", "ETC")
            enumNames(ScheduleNumberType.entries.toTypedArray()) shouldBe listOf(
                "FIRST", "SECOND", "THIRD", "FOURTH", "FIFTH", "SIXTH", "SEVENTH", "EIGHTH", "NINTH", "TENTH",
            )
            enumNames(BankNameType.entries.toTypedArray()) shouldBe listOf(
            ...
            enumNames(BookingStatusType.entries.toTypedArray()) shouldBe listOf(
                "CHECKING_PAYMENT", "BOOKING_CONFIRMED", "BOOKING_CANCELLED", "REFUND_REQUESTED", "BOOKING_DELETED",
            )
        }
    ...
    test("api 경계 마이그레이션 전후로 domain enum request JSON 문자열 값이 호환된다") {
        ...
        withClue("request enum deserialization") {
            objectMapper.readValue("""{"socialType":"KAKAO"}""", MemberLoginRequest::class.java).socialType shouldBe SocialTypeRequest.KAKAO
