...
import com.beat.apis.performance.application.nearestDueDate
...
class PerformanceDetailQueryService(
    ...
    private val scheduleAvailabilityReadPort: ScheduleAvailabilityReadPort,
) {
    @Transactional(readOnly = true)
    fun getPerformanceDetail(performanceId: Long): PerformanceDetailResult {
        val performance = findPerformance(performanceId)
        val schedules = scheduleAvailabilityReadPort.findAllByPerformanceId(performanceId)
        ...
        val minDueDate = if (schedules.isEmpty()) {
            Int.MAX_VALUE
        } else {
    ...
    fun getBookingPerformanceDetail(performanceId: Long): BookingPerformanceDetailResult {
        val performance = findPerformance(performanceId)
        val schedules = scheduleAvailabilityReadPort.findAllByPerformanceId(performanceId).map { schedule ->
            val dueDate = calculateDueDate(schedule.evaluatedAt.toLocalDate(), schedule.performanceDate)
