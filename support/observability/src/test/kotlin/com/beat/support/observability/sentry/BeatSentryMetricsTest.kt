...
class BeatSentryMetricsTest : FunSpec() {
    ...
    init {
        test("wrapper를 통해 통제된 metric 이름과 tag로 metrics를 emit한다") {
            ...
            beatMetrics.gauge(
                "booking.queue_size",
                42.0,
                tags = mapOf("securityLevel" to "internal"),
            )
            beatMetrics.distribution("booking.amount.distribution", 15000.0)

            fakeMetrics.calls shouldBe
                listOf(
        ...
        test("잘못된 metric 이름과 high cardinality tag는 거부한다") {
            ...
            shouldThrow<IllegalArgumentException> {
                beatMetrics.count(
                    ...
                    tags = mapOf("rawUri" to "/api/bookings/42"),
                )
            }
            shouldThrow<IllegalArgumentException> {
                beatMetrics.count(
                    "booking.created.count",
                    tags = mapOf("requestUri" to "/api/bookings/42"),
                )
            }
        }
    }
    ...
    private class FakeMetricsApi : IMetricsApi {
        ...
        override fun count(
            ...
            value: Double?,
            unit: String?,
            parameters: SentryMetricsParameters,
        ) = count(key, value, unit)
