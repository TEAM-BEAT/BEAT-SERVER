...
flowchart TB
    ...
    subgraph AUTH_REDIS_CONFIG["APIs explicit import"]
    ...
    ApisInfraConfig["apis InfraConfig"] --> AuthRedisConfig
```
...
| 모듈      | JPA | AuthRedisConfig | ASYNC | EXTERNAL_CLIENTS | REDIS_CACHE | 이유 |
|---------|-----|-----------------|-------|------------------|-------------|------|
| `apis`  | ✅   | 직접 import       | ✅     | ✅                | ❌           | 사용자 API + 인증 상태 저장 + 비동기 알림 + 외부 연동 |
...
@Import(InfraPersistenceConfig::class, AuthRedisConfig::class)
class InfraConfig
...
`AuthRedisConfig`는 APIs composition root가 직접 import하는 독립 config이므로 marker를 구현하지 않습니다.
...
인증 상태 저장소와 `REDIS_CACHE`의 shared cache 정책은 별도 경계입니다.
...
1. GitHub Actions에서 `rollback-dev.yml`을 수동 실행한다.
    ...
    - branch: "Use workflow from" 드롭다운에서 실행할 브랜치 선택 (보통 `develop`)
2. 실제 복원 대상은 원격 호스트의 `{{ release_root }}/<module>/previous.json`에서 읽는다. 따라서 `N+1` 배포가 `previous.json`에 `N`을 남겼다는 증거가 rollback 전제 조건이다.
...
#### rollback 입력 vs previous.json target image
...
| "Use workflow from" branch | rollback workflow가 사용할 Ansible 코드 checkout 기준. `github.sha`(트리거 시점 HEAD)로 자동 결정되며 별도 입력이 없다. | 잘못된 브랜치는 GHA UI에서 선택 불가. |
