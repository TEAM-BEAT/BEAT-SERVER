...
class RedisRefreshTokenAdapterTest {
    ...
    fun `refresh token을 memberId와 함께 저장한다`() {
        refreshTokenAdapter.save(1L, "refresh-token")
    ...
    fun `저장된 refresh token으로 memberId를 조회한다`() {
        given(refreshTokenRepository.findByRefreshToken("refresh-token"))
            .thenReturn(RefreshTokenRedisHash(1L, "refresh-token"))

        assertEquals(1L, refreshTokenAdapter.findMemberIdByRefreshToken("refresh-token"))
    }

    @Test
    fun `존재하지 않는 refresh token 조회는 비어 있는 결과를 반환한다`() {
        given(refreshTokenRepository.findByRefreshToken("missing")).thenReturn(null)

        assertNull(refreshTokenAdapter.findMemberIdByRefreshToken("missing"))
    }

    @Test
    fun `조회된 refresh token을 삭제한다`() {
        val token = RefreshTokenRedisHash(1L, "refresh-token")
        given(refreshTokenRepository.findById(1L)).thenReturn(java.util.Optional.of(token))

        assertTrue(refreshTokenAdapter.delete(1L))
        verify(refreshTokenRepository).delete(token)
    }

    @Test
    fun `삭제 대상이 없으면 멱등하게 false를 반환한다`() {
        given(refreshTokenRepository.findById(1L)).thenReturn(java.util.Optional.empty())

        assertFalse(refreshTokenAdapter.delete(1L))
