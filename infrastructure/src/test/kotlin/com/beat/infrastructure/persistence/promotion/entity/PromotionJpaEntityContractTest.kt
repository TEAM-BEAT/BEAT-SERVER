package com.beat.infrastructure.persistence.promotion.entity
...
class PromotionJpaEntityContractTest :
    FunSpec({
        test("kotlin jpa plugin은 entity를 JPA를 위해 open하고 인스턴스화 가능하게 유지한다") {
            ...
            // Kotlin documents the no-arg artifact as synthetic; current 2.3.21 bytecode exposes it
            // as
            // hidden/deprecated, so either marker proves this constructor came from the compiler
            // plugin.
            (noArgConstructor.isSynthetic ||
                noArgConstructor.isAnnotationPresent(java.lang.Deprecated::class.java)) shouldBe
                true
        }
    })
