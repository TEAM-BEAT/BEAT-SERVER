...
import com.beat.infrastructure.persistence.promotion.mapper.PromotionPersistenceMapper
...
internal class PromotionRepositoryImpl(
    ...
    override fun lockAll(): List<Promotion> {
        ...
        check(
            promotionJpaRepository.acquireMutationLock(MUTATION_LOCK_NAME, LOCK_TIMEOUT_SECONDS) ==
                1
        ) {
    ...
    override fun findByCarouselNumber(carouselNumber: CarouselNumber): Promotion? =
        promotionJpaRepository
            .findByCarouselNumber(carouselNumber)
            ?.let(promotionPersistenceMapper::toDomain)
