...
@Repository
internal class HomeProjectionQueries(private val dsl: DSLContext) : HomeProjectionReader {

    override fun read(genre: String?, now: LocalDateTime): HomeProjection {
        val performances = findPerformances(genre)
        val performanceDates =
            findPerformanceDates(
    ...
    private fun findPerformanceDates(
        ...
        return dsl.select(
            ...
            .fetch { record ->
                val pid = record[Schedule.PERFORMANCE_ID]!!
                val date = record["performance_date", LocalDateTime::class.java]
                pid to date
            }
            ...
            .let { map ->
                // Need to ensure all requested ids present? Original returns only those with
                // schedules.
    ...
    private fun findPromotions(): List<HomePromotionProjection> {
        val rows =
            dsl.select(
                ...
                .fetch { record ->
                    HomePromotionRow(
                        promotionId = record[Promotion.ID],
                        promotionPhoto = record[Promotion.PROMOTION_PHOTO]!!,
                        performanceId = record[Promotion.PERFORMANCE_ID],
                        redirectUrl = record[Promotion.REDIRECT_URL]!!,
                        isExternal = record[Promotion.IS_EXTERNAL]!!,
                        carouselNumber = record[Promotion.CAROUSEL_NUMBER]!!,
                    )
                }

        // Preserve original sort by carouselNumber enum order (ONE, TWO, THREE...)
        // CarouselNumber number ordering: use lexical via enum name ordering? Original used
        // CarouselNumber.number int.
        // We replicate by mapping to enum and sorting by enum's number via valueOf.
        return rows
            .sortedBy { row ->
                try {
                    com.beat.domain.promotion.model.CarouselNumber.valueOf(row.carouselNumber)
                        .number
                } catch (_: Exception) {
                    Int.MAX_VALUE
                }
